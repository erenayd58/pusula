...
2. *Netler:* ders satırları (branşta tek satır). Doğru ve Yanlış `dense` adımlayıcılarla tek satırda ("Doğru − 16 + Yanlış − 2 +"); **Boş otomatik** = `exam_question_count − D − Y`, yalnızca metin ("Boş 2"), dokununca / Enter'la düzenlenir (C4); D+Y+B sayıyı aşarsa satır hatası "Bu derste 20 soru var". Ders kartı telefonda iki satır (~100 px; 6 ders 390×844'te iki ekran), ≥ sm tek satır. Satır sonunda `net` canlı; altta yapışkan özet "Toplam 90 soru · Net 71,33" (`aria-live`), alt menünün üstünde (`--nav-bottom`). Ders soru sayısı boşsa Boş elle. Klavye: Tab alanlar (Boş düğme olarak sırada), ↑↓ sayı, Enter sonraki adım, Esc vazgeç (Boş girdisi açıkken Enter/Esc yalnızca girdiyi kapatır).
...
**Düzeltme (2026-09-19, görüntü incelemesi):** sihirbaz 2. adımı ders kartı tek satır (`NumberStepper dense`, Boş metin; ~250 → ~100 px); `LineChart` `viewBox` genişliği kabın ölçülen genişliği (telefonda 11 px metin ve nokta boyutu korunur, etiket sayısı 3–6) ve `secondaryToggle` ile ders çipleri < md "Dersleri göster" arkasında (grafik ilk ekranda); öğrenci/veli `main` alt boşluğu ve sihirbaz yapışkan özeti `--nav-bottom` token'ından (çubuk + (+) + safe-area); detay ders tablosu telefonda dar dolgu (358 px'e sığar). e2e: Boş `getByRole("button", { name: "Boş 2" })`, telefonda "Dersleri göster" tıklanır.
...
**Uygulama notu (2026-09-19, Parça 2 tamamlandı):** belgedeki dosya listesi uygulandı; farklar ve ekler: `mistakes/lib/storage.ts` (bucket + yol; istemci/sunucu ortak) ve `lib/filters.ts` (`parseMistakeFilters`: liste filtresi URL parametresi `?subject=&status=&reason=`, sunucu süzer; neden çipleri "Nedene göre" katlanır bölümünde). `Mistake` tipi deneme başlığı taşımaz (`mock-exams` tablosuna embed yok); detayda "Denemeye git" yalnızca URL. Öğrenci detay sayfası (`[mistakeId]`) `?edit=1` ile formu dolu açar (neden / konu / not; fotoğraf değişmez). Fotoğraf yüklemede ilerleme çubuğu yok (supabase-js standart yükleme olay vermez); düğme "Fotoğraf yükleniyor…" durumu. `MistakeDetail`, `MistakeStatusButton` / `MistakeDeleteButton` ek bileşenler. `getSubjectProgress` (K2 ders tablosu) Parça 1'deki TS hesabında kaldı; `v_student_mock_subject_stats` yalnızca analytics `getStrategyContext` okur. Seed'de Üslü İfadeler `completed` (belgede `mastered` yazıyordu) → sebep "Tamamlandı işaretli ama son 3 denemenin 2'sinde yanlış · yanlış defterinde 1 soru" (iki sinyal birleşik); e2e buna göre. pgTAP `230` depo silmeyi politika varlığıyla test eder (yerel `storage.protect_delete` doğrudan DELETE'i engeller; gerçek silme e2e'de). `LineChart` `hint` prop'u (veli "dokunun"); `LastResultCards.href` ve `ResultList.basePath` isteğe bağlı (veli bağlantısız). `deleteStudent` depo temizliği owner'ın kurumunu kullanır (`can_delete_student` aynı kurumu garanti eder). e2e `mistakes.spec.ts` üç test (defter akışı iki projede; veli; seed uyarı bağlama masaüstünde). Ekran görüntüleri `docs/tasarim/uygulama-6/` (7 yeni).

**Düzeltme (2026-09-19, Parça 2 görüntü incelemesi):** `lib/format.formatNamePossessive` (ad + ilgi hali: `Ayşe'nin`, `Mehmet'in`, `Can'ın`; kaynaştırma "n", ünlü uyumu; birim testli) — veli kabuğu başlığı ve Denemeler sekmesi bunu kullanır. Veli Özet: "yakında eklenecek" yer tutucu metni kaldırıldı; kart varsa yalnızca kartlar, yoksa "Henüz özet yok" boş durumu. Alt menü: öğrenci/veli `main` alt boşluğu (`--nav-bottom` + 24 px) tüm sayfalarda zaten var (ölçüm: içerik alt kenarı alt menünün 34 px, (+) düğmesinin 16 px üstünde); görüntülerdeki "kapatma" Playwright tam sayfa yakalama artefaktıydı — `scripts/screenshots.mjs` yakalamadan önce sabit alt menüyü belge altına sabitler (`nav.fixed → absolute`; yalnızca görüntü için).
