...
>
> **Durum:** tasarım onaylandı (2026-09-19; §7 kararları önerildiği gibi, D1–D17). **İki parça da uygulandı ve faz kapatıldı (2026-09-19, dal `faz-7-kaynaklar`)**; uygulama notları §2'de parça sonlarında, belge güncellemeleri 01/02/03/04/07/08/09/10'a işlendi (02 karar #49).
...
**Kabul:** koç 40 testlik kitabı tek partide tanımlar, konuya eşler, 2 öğrenciye tek tıkla atar (e2e); öğrenci testi işaretleyince hızlı kayıt ön dolu açılır ve kaynak yüzdesi güncellenir (e2e; 01 Faz 7 kabulü); plan görevi `section` türü hızlı kayıtla tamamlanır ve kayıt iki bağı taşır (e2e + pgTAP); öğrencinin eklediği kaynak yalnızca kendisine görünür, koç "Katalogda tut" yapar (pgTAP + e2e); benzer ad önerisi (birim + e2e); `generateSections`, `resourceProgress`, `similarTitles`, `sectionsToPoolItems` birim testli; `pnpm check` + `pnpm db:test` yeşil. ✅ Uygulandı (2026-09-19).

**Uygulama notu (Parça 1):** belgedeki dosya listesi uygulandı; farklar ve ekler: `resource-actions.tsx` (`ResourceActions` başlık eylemleri: Düzenle / Katalogda tut / Kaldır; `AssignedStudents` atanan listesi + atamayı kaldır); kitap formu tek tip `resourceFormSchema` (yeni + düzenleme; düzenlemede `batches` boş) — `as` zorlaması yok; `SectionEditor` tek test düzenlemede `useWatch`; `StudentResourceDetail` koç K2'de `readOnly` ve hızlı kayıt sağlayıcısı yokken (`QuickLogContext` null) dokunuşsuz; `SectionRow.resourceId` (K2 gruplama); `listResourceTemplates` (koç formunda `?template=`); `question-log` kayıt listeleri `resource_sections(title, resources(title))` embed'iyle "Kitap · Test 12" etiketi (RLS okur); e2e tek masaüstü testinde tüm akış (`desktop-chromium`; kurum kataloğu paylaşımlı olduğu için zaman damgalı adlar, teardown `deleteE2EResources`). Test sayfa aralığı örneği: sayfa 10'dan 4'er → Test 2 = 14–17.
...
**Kabul:** koç bir YouTube liste bağlantısı yapıştırınca videolar başlık/süre/sırayla gelir (yerel elle + birim fixture; 01 Faz 7 kabulü); anahtar yokken elle liste ve video eklenir (e2e); öğrenci uygulama içinde izler, "İzledim" yüzdeyi ve bağlı plan görevini günceller (e2e + pgTAP); havuz `videos`/`resources` kategorileri ve zayıf konu öne çıkarma (birim); bilgi eksiği / başlanmamış konuya eşli video varsa öneri `video` türünde (birim + e2e); owner şablonu kataloglarla kopyalar (pgTAP + e2e); `parseYoutubeUrl`, `parseIsoDuration`, `playlistProgress`, `videosToPoolItems`, `alertToTask(media)` birim testli; `pnpm check` + `pnpm db:test` yeşil. ✅ Uygulandı (2026-09-19).

**Uygulama notu (Parça 2):** YouTube istemcisi `src/lib/youtube/client.ts`'te (anahtar ve `fetch` parametre; `server-only` sarmalayıcı `features/videos/server/youtube.ts` yalnızca `serverEnv.youtubeApiKey` verir) — birim test `server-only` import etmeden sahte `fetch` ile 2 sayfa + süre + hata kodlarını doğrular; `youtubeErrorMessage` Türkçe metinleri tek yerde. Video sırası elle değiştirilmez (belge: kapsam dışı). `video_playlists` tekilliği düzeltmesi (`faz7b_playlist_unique_fix`): `unique nulls not distinct` elle listeleri de tekilleştiriyordu (seed'deki elle liste yüzünden ikinci elle liste 23505) → kısmi tekil indeks yalnızca `youtube_playlist_id` dolu satırlarda. `mark_video_watched` `p_student_id` isteğe bağlı (koç öğrencisi için). Öneri motorunda `getTopicMedia` iki görünümü okur ve `student_modules` ile kapalı modülleri eler; analytics planner'ı import etmediği için `videoMinutes` kuralı (`max(5, ceil(s/60))`, süre yoksa `link_minutes`) sorgu katmanında yinelenir. `copy_curriculum_template` geçici tablolarla (`pg_temp`, `on commit drop`; aynı transaction'da ikinci çağrı için önce düşürülür); YouTube listesi kopyada bağlantısız (elle) liste olur (tekillik). Şablon seçici `/coach/templates?template=` yalnızca birden fazla şablon varken görünür; Takvim/Konular bağlantıları parametreyi korur; kopya formu varsayılan sezonu `currentSeason()`'ın bir sonrası. `write-local-env.mjs --force` artık `YOUTUBE_API_KEY` gibi ek anahtarları korur. e2e `videos.spec.ts` tek masaüstü testinde tüm akış; teardown `deleteE2EPlaylists` + `deleteE2ETemplates`. Tam paket: `test:e2e` 83 geçti / 9 atlandı.
